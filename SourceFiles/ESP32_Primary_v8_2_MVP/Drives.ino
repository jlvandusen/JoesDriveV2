...
//  Input1 = Potentiometer, Input2 = IMU
  ...
  #ifdef EnableFilters
    target_pos_S2S = Filter_target_pos_S2S.filter(target_pos_S2S);
  #endif
...
//  Input2 = (receiveIMUData.roll*-1)- IMUDeadzone;   // ****add a bit to the IMU to get the real middle point
  ...
  #ifdef EnableFilters
  Input2 = Filter_Input2.filter(Input2);
  #endif
  ...
  #ifdef EnableFilters
    Input1 = Filter_Input1.filter(Input1);
  #endif
...
//    analogWrite(Drive_pwm, (0-255));
  ...
  #ifdef EnableFilters
    Input3 = Filter_Input3.filter(Input3);
  #endif
  ...
  #ifdef EnableFilters
    Setpoint_Drive_In = Filter_Setpoint_Drive_In.filter(Setpoint_Drive_In);
  #endif

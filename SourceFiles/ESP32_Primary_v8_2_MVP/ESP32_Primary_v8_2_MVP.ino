...
// #define revDrive
// #define revGyro
#define EnableFilters  // Used to quiet noisy pin feedback
...
#include <nvs_flash.h> // for wiping NVram
  ...
  Ewma Filter_target_pos_S2S(0.2);  // More smoothing - less prone to noise, but slower to detect changes
  Ewma Filter_Input2(0.2);  // More smoothing - less prone to noise, but slower to detect changes
  Ewma Filter_Input3(0.2);  // More smoothing - less prone to noise, but slower to detect changes
  Ewma Filter_Input1(0.2);  // More smoothing - less prone to noise, but slower to detect changes
  Ewma Filter_Setpoint_Drive_In(0.2);  // More smoothing - less prone to noise, but slower to detect changes
  Ewma Filter_leftStickx(0.01);  // More smoothing - less prone to noise, but slower to detect changes
  Ewma Filter_leftSticky(0.01);  // More smoothing - less prone to noise, but slower to detect changes
#endif
...
struct SEND_DATA_STRUCTURE_32u4{  
  bool driveEnabled;
  int8_t domeSpin;
  bool moveL3; // domeServoMode
  bool moveR3; // reverseDrive
  int8_t leftStickX;
  int8_t leftStickY;
  int8_t soundcmd;
  int8_t psiFlash;
...
float Drive_IMUDeadzone = .05;   // target fluctuating between for IMU
float S2S_potDeadzone = .05; // 3

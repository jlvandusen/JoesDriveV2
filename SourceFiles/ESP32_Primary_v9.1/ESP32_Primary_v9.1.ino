 ...
 * ESP32 Primary v9.1 — enhanced ESPNOW mirroring with reliable send-queue
 * (Primary device)
 */
...
// #include <avr/wdt.h>  // enable watchdog service
...
// ----- Feature toggles -----
bool ENABLE_TELEMETRY   = false;   // default OFF; toggle via "SET TELEMETRY ON|OFF"
...
void applyTelemetrySetting() {
  ...
  telemetryDue = false;
  if (ENABLE_TELEMETRY) telemetryTicker.attach_ms(500, telemetryTick);
}
// #endif
...
void setup() {
  ...
  // wdt_enable(WDTO_4S); // 4-second watchdog (closest to 5s)
  ...
  if (esp_now_init() != ESP_OK) {
    ...
    // add remote peer (receiver)
    esp_now_peer_info_t peerInfo = {};
    memcpy(peerInfo.peer_addr, remoteMAC, 6);
    peerInfo.channel = 0;
    peerInfo.encrypt = false;
    if (esp_now_add_peer(&peerInfo) != ESP_OK) {
      LOGLN("[ESPNOW] Failed to add peer");
    }

    // telemetry via ticker flag; send from loop() only when enabled
    applyTelemetrySetting();   // respects ENABLE_TELEMETRY (default OFF)
  }
#endif
  ...
  LOGF("T: %lu INFO: Loaded Cal - Pot:%d Pitch:%.2f Roll:%.2f | PID[P]=%.2f/%.2f/%.2f | S2S[Pk2]=%.2f/%.2f/%.2f\n",
...
// Auto-leveling on first boot
if (!preferences.isKey("potOffset")) {
  Serial.println("[AUTO-CENTER] First boot detected. Waiting for level...");
  unsigned long start = millis();
  float pitchSum = 0, rollSum = 0;
  int potSum = 0, samples = 0;

  while (millis() - start < 5000) { // Wait and collect for 5 seconds
    if (recIMU.receiveData()) {
      pitchSum += receiveIMUData.pitch;
      rollSum += receiveIMUData.roll;
  ...
  if (samples > 0) {
    ...
    // Check if roll is within ±0.50° of level
    if (fabs(avgRoll) <= 0.50f) {
      pitchOffset = avgPitch;
      rollOffset  = avgRoll;
      potOffset   = potSum / samples;
      ...
      #ifdef USE_ESPNOW
        sendStatusESPNOW("[AUTO-CENTER COMPLETE]");
      #endif
    } else {
      // Fallback: Use defaults
      potOffset = defaultPOT;
      pitchOffset = 0.0f;
      rollOffset  = 0.0f;
      ...
      Serial.printf("[AUTO-CENTER] Skipped: Roll not level (%.2f°). Using defaults.\n", avgRoll);
      #ifdef USE_ESPNOW
        sendStatusESPNOW("[AUTO-CENTER SKIPPED: USING DEFAULTS]");
      #endif
    }
  } else {
    // No IMU data fallback
    potOffset = defaultPOT;
    pitchOffset = 0.0f;
    rollOffset  = 0.0f;
    ...
    #ifdef USE_ESPNOW
      sendStatusESPNOW("[AUTO-CENTER FAILED: USING DEFAULTS]");
    #endif
  }
}
  ...
  // // Auto-leveling on first boot
  // if (!preferences.isKey("potOffset")) {
  //   LOGLN("[AUTO-CENTER] First boot detected. Leveling...");
  //   unsigned long start = millis();
  //   float pitchSum = 0, rollSum = 0;
  //   int potSum = 0, samples = 0;
  ...
  //       LOGF("[AUTO-CENTER] Saved: Pot=%d Pitch=%.2f Roll=%.2f\n", potOffset, pitchOffset, rollOffset);
  //     } else {
  //       LOGF("[AUTO-CENTER] Skipped: Roll not level (%.2f°)\n", avgRoll);
  //     }
  //   } else {
  //     LOGLN("[AUTO-CENTER] Failed: No IMU data collected.");
  //   }
  // }
  ...
  LOGLN("JOE'S DRIVE V8.9b — ESP32 MASTER — SER — FULLY READY");
...
void handleButtonActions() {
  ...
  // Toggles
  if (EDGE_PRESSED(buttonsR.ps,  prevR.ps)) { enableDrive = !enableDrive; sendTo32u4Data.driveEnabled = enableDrive; LOGF("T: %lu enableDrive %s\n", millis(), enableDrive ? "ON" : "OFF"); emitSound(SOUND_TOGGLE); }
  if (EDGE_PRESSED(buttonsL.l3,  prevL.l3)) { DomeServoMode = !DomeServoMode; sendTo32u4Data.moveR3 = DomeServoMode; LOGF("T: %lu Dome Servo Mode %s\n", millis(), DomeServoMode ? "ON" : "OFF"); emitSound(SOUND_TOGGLE); }
  if (EDGE_PRESSED(buttonsR.l3,  prevR.l3)) { reverseDrive = !reverseDrive; sendTo32u4Data.moveL3 = reverseDrive; LOGF("T: %lu Reverse Drive %s\n", millis(), reverseDrive ? "ON" : "OFF"); emitSound(SOUND_TOGGLE); }
  if (EDGE_PRESSED(buttonsR.cross, prevR.cross)) { autoBalance = !autoBalance; LOGF("T: %lu Auto Balance %s\n", millis(), autoBalance ? "ON" : "OFF"); emitSound(SOUND_TOGGLE); }
...
void handleSerialCommands(String cmd) {
  ...
  if (cmd.startsWith("SET ")) {
    ...
    // Feature toggles
    else if (param == "REVERSE_S2S") REVERSE_S2S = boolVal;
    else if (param == "ENABLE_ROLL_BIAS") ENABLE_ROLL_BIAS = boolVal;
    else if (param == "TELEMETRY") {
      ENABLE_TELEMETRY = boolVal;   // ON/OFF, TRUE/FALSE, 1/0
    ...
    String status = String("[CONFIG] REVERSE_S2S=") + String(REVERSE_S2S ? "ON" : "OFF") +
                    ...
                    " TELEMETRY=" + String(ENABLE_TELEMETRY ? "ON" : "OFF");
    LOGF("%s\n", status.c_str());
  }
    ...
    #ifdef USE_ESP32NOW
    sendStatusESPNOW("[RESET COMMAND SENT]");
    #endif
...
void printActiveDebugFlagsOnce() {
  ...
  if (!DEBUG_ALL && !DEBUG_COMPACT && !DEBUG_CALIBRATION && !DEBUG_JOYSTICK &&
      !DEBUG_S2S_MODE && !DEBUG_MOTOR_OUTPUT && !DEBUG_IMU_RAW) {
    LOG("(none)");
  }
  LOGLN("");
}
...
void loop() {
  #ifdef USE_ESPNOW
  // Pump ESPNOW send queue in task context
  pumpEspNowSends();

  // Telemetry (if enabled) — enqueue from task context
  if (ENABLE_TELEMETRY && telemetryDue) {
    telemetryDue = false;
    sendTelemetryNow();
  }

  // Process commands typed into PRIMARY Serial Monitor (USB)

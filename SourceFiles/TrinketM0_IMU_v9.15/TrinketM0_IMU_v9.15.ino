...
struct SEND_DATA_STRUCTURE {
  float rawX;      // Raw acceleration X
  float rawY;      // Raw acceleration Y
  float rawZ;      // Raw acceleration Z
  float pitch;     // Calculated pitch angle (degrees)
  float roll;      // Calculated roll angle (degrees)
};
...
void readmpu() {
  ...
  sendIMUData.rawX = accel.acceleration.x;
  sendIMUData.rawY = accel.acceleration.y;
  sendIMUData.rawZ = accel.acceleration.z;

  // Calculate roll and pitch in degrees
  float X = accel.acceleration.x;
  float Y = accel.acceleration.y;
  float Z = accel.acceleration.z;
  sendIMUData.roll = atan2(Y, Z) * 180 / PI;
  sendIMUData.pitch = atan2(-X, sqrt(Y*Y + Z*Z)) * 180 / PI;

#ifdef debugMPU
  static unsigned long lastDebug = 0;
  if (millis() - lastDebug >= 500) {
    Serial.printf("Raw: X=%.2f Y=%.2f Z=%.2f | Roll=%.2f Pitch=%.2f\n",
                  sendIMUData.rawX, sendIMUData.rawY, sendIMUData.rawZ,
                  sendIMUData.roll, sendIMUData.pitch);
    lastDebug = millis();
  }
#endif
}

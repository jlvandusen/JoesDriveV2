...
bool debug32u4Status    = false; // Set to true to enable status debug
...
// ESPNOW struct for Dome communication
typedef struct struct_message {
  int psi;       // PSI trigger (0 or 1)
  byte btn;      // Optional button state
  float bat;     // Battery voltage from Dome
  int anim;      // Animation code (1–99)
} struct_message;

struct_message domeDataOut;  // Data we send to Dome
struct_message domeDataIn;   // Data we receive from Dome

struct Receive32u4Status {
  int domeTilt;        // Dome tilt angle or position
  bool soundPlaying;   // True if sound is currently playing
  int lastSoundID;     // Last sound triggered
  int domeDirection;   // Dome rotation direction (-1, 0, 1)
};

Receive32u4Status statusFrom32u4;
EasyTransfer recv32u4; // New EasyTransfer instance for receiving
...
inline void pumpEspNowSends() {
  ...
  // 1. Send domeDataOut every 100ms
  static unsigned long lastStructSend = 0;
  if (USE_ESPNOW && (now - lastStructSend >= 100)) {
    esp_err_t result = esp_now_send(remoteMAC, (uint8_t*)&domeDataOut, sizeof(domeDataOut));
    if (result != ESP_OK) {
      Serial.println("[ESPNOW] Struct send failed");
    }
    lastStructSend = now;
  }

  // 2. Handle queued text messages (existing logic)
  if (inFlight) return;
...
void setup() {
    ...
    recv32u4.begin(details(statusFrom32u4), &Serial2);
...
void triggerAnimation(int animCode) {
  domeDataOut.anim = animCode; // 1–99
  Serial.printf("[ANIM] Sending animation %d to Dome\n", animCode);
}


void onDataRecv(const uint8_t *mac, const uint8_t *incomingData, int len) {
  if (len == sizeof(domeDataIn)) {
    memcpy(&domeDataIn, incomingData, sizeof(domeDataIn));
    Serial.printf("[DOME STATUS] Battery=%.2fV | PSI=%d | Anim=%d\n",
                  domeDataIn.bat, domeDataIn.psi, domeDataIn.anim);
  } else {
...
// Always check for Serial commands
  ...
  if (tuning != TuningSession::NONE) {
    ...
    if (recv32u4.receiveData()) {
      updateDomeAnimationFromStatus();
      debug32u4StatusPrint(); // Only prints if flag is true
    }
...
/* ------------------- DOME Animation Routines ------------------- */
void updateDomeAnimationFromStatus() {
  // PSI flicker when sound is playing
  domeDataOut.psi = statusFrom32u4.soundPlaying ? 1 : 0;

  // Default animation
  domeDataOut.anim = 0;

  // Map domeDirection to animations
  if (statusFrom32u4.domeDirection > 0) {
    domeDataOut.anim = 2; // CW rotation → Anim 2
  } else if (statusFrom32u4.domeDirection < 0) {
    domeDataOut.anim = 3; // CCW rotation → Anim 3
  }

  // Map tilt to animation if tilt exceeds threshold
  if (abs(statusFrom32u4.domeTilt) > 5) {
    domeDataOut.anim = 10; // Tilt → Anim 10
  }

  // Optional: Map lastSoundID to special animation
  if (statusFrom32u4.lastSoundID >= 40 && statusFrom32u4.lastSoundID <= 50) {
    domeDataOut.anim = 8; // Special sound → Anim 8
  }
}
...
void handleSerialCommands(String cmd) {
  ...
  else if (cmd == "SHOW 32U4") {
    Serial.printf("[32u4 STATUS] Tilt=%d | SoundPlaying=%d | LastSound=%d | DomeDir=%d\n",
                  statusFrom32u4.domeTilt,
                  statusFrom32u4.soundPlaying,
                  statusFrom32u4.lastSoundID,
                  statusFrom32u4.domeDirection);
  }
  
  else if (cmd == "DEBUG 32U4 ON") {
    debug32u4Status = true;
    Serial.println("[DEBUG] 32u4 status debug enabled");
  }
  
  else if (cmd == "DEBUG 32U4 OFF") {
    debug32u4Status = false;
    Serial.println("[DEBUG] 32u4 status debug disabled");
  }
...
void debug32u4StatusPrint() {
  static unsigned long lastDebug = 0;
  if (!debug32u4Status) return; // Exit if debugging is disabled

  if (millis() - lastDebug >= 1000) { // Print every 1 second
    lastDebug = millis();
    Serial.printf("[32u4 STATUS] Tilt=%d | SoundPlaying=%d | LastSound=%d | DomeDir=%d\n",
                  statusFrom32u4.domeTilt,
                  statusFrom32u4.soundPlaying,

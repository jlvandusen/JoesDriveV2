...
#ifndef debugENC
  if (domeServoPWM > 15 && enableDrive && !domeServoMode) {
#else
  if (domeServoPWM > 15 && !domeServoMode) {
#endif
    digitalWrite(domeMotor_pin_A, LOW);
    digitalWrite(domeMotor_pin_B, HIGH); // Motor 1 Forward
#ifndef debugENC
  } else if (domeServoPWM < -15 && enableDrive && !domeServoMode) {
#else
  } else if (domeServoPWM < -15 && !domeServoMode) {
#endif
    digitalWrite(domeMotor_pin_A, HIGH);
...
void domeServoMovement() {
  ...
  if (Output_domeSpinServoPid >10 && enableDrive) {
    digitalWrite(domeMotor_pin_A, LOW);
    digitalWrite(domeMotor_pin_B, HIGH); // Motor 1 Forward
    analogWrite(domeMotor_pwm,abs(domeServoPWM));
  } else if (Output_domeSpinServoPid <-10 && enableDrive) {
    digitalWrite(domeMotor_pin_A, HIGH);
    digitalWrite(domeMotor_pin_B, LOW); // Motor 1 Backward

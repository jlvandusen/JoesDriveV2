...
//  musicPlayer.useInterrupt(VS1053_FILEPLAYER_PIN_INT);  // DREQ int
  ...
  pinMode(hallEffectSensor_Pin, INPUT_PULLUP);  // pullup resistor built into 32u4 to lesson noise
  pinMode(domeMotor_pwm, OUTPUT);    // Speed Of Motor1 on Motor Driver 1
  pinMode(domeMotor_pin_A, OUTPUT);  // Direction checks with ENC on Motor
  pinMode(domeMotor_pin_B, OUTPUT);
  #ifndef UseHallMonitor
    domeCenterSet = true;   // if we dont use Hall Monitor - set current state to center for the dome
    myEnc.write(749); //740
  #endif
}
